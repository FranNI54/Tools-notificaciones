...
class Validation_callables extends MY_Model {
	...
	public function _check_password_strength( $password )
	{
		...
		if( config_item('max_chars_for_password') > 0 )
			$error .= '<li>No más de ' . config_item('max_chars_for_password') . ' caracteres</li>';
		...
		if( config_item('min_digits_for_password') > 0 )
		{
			...
			$error .= '<li>' . config_item('min_digits_for_password') . ' número' . $plural . '</li>';
		}
		...
		if( config_item('min_lowercase_chars_for_password') > 0 )
		{
			...
			$error .= '<li>' . config_item('min_lowercase_chars_for_password') . ' letra minúscula' . $plural . '</li>';
		}
		...
		if( config_item('min_uppercase_chars_for_password') > 0 )
		{
			...
			$error .= '<li>' . config_item('min_uppercase_chars_for_password') . ' letra mayúscula' . $plural . '</li>';
		}
		...
		if( config_item('min_non_alphanumeric_chars_for_password') > 0 )
		{
			...
			$error .= '<li>' . config_item('min_non_alphanumeric_chars_for_password') . ' caracteres no alfanuméricos' . $plural . '</li>';
		}
