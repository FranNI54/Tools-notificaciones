...
class User extends MY_Controller
{
	...
	public function create(){
		...
		$validation_rules = [
			[
				...
				'errors' => [
					'is_unique' => 'Nombre de usuario ya en uso'
				]
			],
			[
				'field' => 'passwd',
			...
			[
				...
				'rules'  => 'trim|required|valid_email|is_unique[' . db_table('user_table') . '.email]',
				'errors' => [
					'is_unique' => 'El mail ya esta en uso'
				]
			],
			[
				'field' => 'auth_level',
		...
		else
		{
			...
			if( $this->db->affected_rows() == 1 ){
				$data["message"]= 'Usuario ' . $user_data['username'] . ' fue creado con éxito.';
				$data["title"]= "Felicidades!";
				$this->load->view('templates/header', $data);
	...
	public function update($id){
		...
		$data["user"]=$this->examples_model->all($id);	
		
		unset($_POST["token"]);
		if(isset($_POST["email"])||isset($_POST["username"])){
			$user_data= $_POST;
			if(!$user_data["passwd"]||$user_data["passwd"]==""){
				$user_data["passwd"]="Password123";
			}
		}else{
			$user_data=array();
		}
		...
		else
		{
			...
			if( $this->db->affected_rows() == 1 ){
				$data["message"]= 'Usuario ' . $user_data['username'] . ' fue actualizado con éxito.';
				$data["title"]= "Felicidades!";
				$this->load->view('templates/header', $data);
				$this->load->view('user/message', $data);
				$this->load->view('templates/footer');
	...
	public function admin(){
		...
		$data["usuarios"]= $this->examples_model->all();
		$data["token"]=$this->tokens->token();
